...
export const initLineup = () => {
  ...
  const getScrollEnd = () => {
    ...
    // 画面幅がMD（768px）未満なら1.75倍、以上なら1倍（そのまま）にする
    const multiplier = window.innerWidth < BREAKPOINTS.MD ? 1.75 : 1;
  ...
  mm.add({
    ...
    // scrollTriggerのscrub設定（PC: 少し遅らせる, SP: 1対1の設定）
    const scrubValue = isPc ? 1 : true;
    ...
    // pc表示時はアニメーション対象に filter を追加
    if (isPc) {
      commonFrom.filter = 'blur(8px)';
      commonTo.filter = 'blur(0px)';
    }
